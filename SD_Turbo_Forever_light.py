...
# Path to the bundled pip.exe in the _internal folder
bundled_pip_path = os.path.join(os.path.dirname(sys.executable), "_internal", "pip.exe")
bundled_install_path = os.path.join(os.path.dirname(sys.executable), "_internal")

# Check if pip.exe exists and print a warning if it doesn't
if not os.path.exists(bundled_pip_path):
    print(f"Warning: {bundled_pip_path} not found. Ensure that pip.exe is bundled properly.")
    sys.exit(1)

# Set environment variables to store models in _internal
os.environ["PYTHONUSERBASE"] = bundled_install_path  # For user-level package installations
os.environ["TRANSFORMERS_CACHE"] = bundled_install_path  # Cache for transformers models
os.environ["HF_HOME"] = bundled_install_path  # Hugging Face home for models

def install_package(package_name):
    try:
        print(f"Installing {package_name} using {bundled_pip_path}...")
        # Run pip.exe with the --target option to install in the _internal directory
        process = subprocess.run(
            [bundled_pip_path, "install", package_name, "--upgrade", "-v", "--target", bundled_install_path],
            ...
            check=True,
            shell=False   # Ensure subprocess runs directly in Windows
        )
        print(process.stdout)  # Print output to help debug
        print(f"{package_name} installed successfully in {bundled_install_path}.")
...
def load_initial_image():
    image_name = 'sd_turbo_forever.jpg'

    # Check if the image is in the current directory
    if os.path.exists(image_name):
        image_path = image_name
    else:
        # Check in the internal folder used by PyInstaller
        bundled_path = os.path.join(os.path.dirname(sys.executable), image_name)
        if os.path.exists(bundled_path):
            image_path = bundled_path
        else:
            print(f"Initial image not found. Please make sure '{image_name}' is in the working directory or bundled folder.")
            return
